...
CKEDITOR.plugins.add('wpmore', {
    ...
    getPlaceholderCss: function getPlaceholderCss() {
        return 'img.cke_wordpress_more' + '{' + 'background-image: url(' + CKEDITOR.getUrl(this.path + 'images/more_bug.gif') + ');' + 'background-position: right center;' + 'background-repeat: no-repeat;' + 'clear: both;' + 'display: block;' + 'float: none;' + 'width: 100%;' + 'border-top: #999999 1px dotted;' + 'height: 10px;' + '}';
    },
            ...
            exec: function exec() {
                ...
                for (var i = 0, len = images.count(); i < len; i++) {
                    ...
                    if (img.hasClass('cke_wordpress_more')) {
                        var msg = 'The document already contains a more. ' + 'Do you want to proceed by removing it first?';
                        if (confirm(msg)) {
                            img.remove();
                            break;
                        } else {
